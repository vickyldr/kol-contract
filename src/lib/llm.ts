...
export async function parseContractInfo(
  ...
  const system = [
    ...
    "2) 格式校验（重要！别只看有没有@）：",
    "   · 区分【邮箱】和【社媒链接】：邮箱 = 名字@邮箱域名（如 gmail.com / qq.com / naver.com），没有路径(/)、不含社媒平台域名；社媒链接/账号 = 含社媒平台域名（instagram.com / tiktok.com / youtube.com / t.me 等）或以 @用户名 形式出现或带路径(/)。",
    "   · socialAccount / kolLink 只能放社媒链接或 @用户名；platform 只能放平台名（Instagram/TikTok/YouTube/Telegram）。",
    "   · 【绝对不能】把邮箱、银行账号、姓名、地址放进 socialAccount/kolLink/platform。邮箱只放邮箱类/PayPal 邮箱类字段。",
    "   · 没有真正的社媒链接时，把 socialAccount、kolLink、platform 都放进 missing（合并成一条：“请提供红人的社媒账号或主页链接”），不要硬塞。",
