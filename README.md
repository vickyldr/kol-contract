...
2. **合同记录 → 新建记录**：
   - 只需选产品。**模板由 AI 自动选**：按收款方式 + 国家——日本→日文模板、韩国→韩文模板、其余→英文（英文里再按 PayPal/Payoneer/银行，银行分 IBAN/SWIFT）。需要时可「手动更换」。
   - **① 一键智能填充**：把红人给的所有信息（姓名、链接、单价、银行/PayPal 收款信息…）一股脑粘贴进去，点一下——AI 先自动选模板，再识别并填到对应栏位；缺失或对不上的会列出来问你，补充后再点一次或手动改即可。
